...
class SocialMediaBot:
    ...
    async def start(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        welcome_text = """
🎥 <b>Ijtimoiy Tarmoq Video Yuklovchi Bot</b>
...
Videoni yuborish uchun shunchaki linkni yuboring! 👇
        """
    ...
    async def handle_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not result['success']:
            await status_msg.edit_text(
                f"❌ Video yuklab olinmadi\n\n"
                f"Sabab: {result['error']}\n\n"
                f"💡 Iltimos, linkni tekshiring yoki boshqa video yuboring."
            )
            return
        ...
        try:
            ...
            keyboard = InlineKeyboardMarkup([
                [InlineKeyboardButton("🎵 Musiqa", callback_data=f"music_{chat_id}")]
            ])
    ...
    async def handle_music_button(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        await query.answer("🔍 Musiqa qidirilmoqda...")

        chat_id = int(query.data.split('_')[1])
        video_url = context.user_data.get(f'video_url_{chat_id}')

        if not video_url:
            await query.message.reply_text("❌ Video topilmadi. Qayta yuklang.")
            return

        search_msg = await query.message.reply_text("🎵 Audio ajratilmoqda...")
        ...
        try:
            ...
            await search_msg.edit_text("🔍 Musiqa tanib olinmoqda...")
            ...
            if music_info['success']:
                ...
                await search_msg.edit_text(f"🔍 <b>{search_query}</b> qidirilmoqda...", parse_mode='HTML')
                
                results = await self.search_youtube_music(search_query)
                
                if results:
                    response = f"🎵 <b>{music_info['artist']} — {music_info['title']}</b>\n\n"
                    ...
                    response += f"\n@ska4at_bot orqali istagan musiqangizni tez va oson toping!"
                    
                    buttons.append([InlineKeyboardButton("🎞 Video", url=video_url)])
                    buttons.append(row)
                    ...
                    await search_msg.edit_text("❌ Hech qanday musiqa topilmadi.")
            else:
                await search_msg.edit_text(
                    f"❌ Musiqa topilmadi\n\n"
                    f"Sabab: {music_info.get('error', 'Noma\'lum xato')}\n\n"
                    f"💡 Ba'zi videolarda musiqa aniqlanmasligi mumkin."
                )
        except Exception as e:
            await search_msg.edit_text(f"❌ Xato yuz berdi: {str(e)}")
    ...
    async def handle_music_selection(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not results or index >= len(results):
            await query.message.reply_text("❌ Ma'lumotlar eskirgan. Qayta qidirib ko'ring.")
            return
        ...
        try:
            ...
            if os.path.exists(audio_path):
                ...
                await status_msg.edit_text("❌ Yuklab olishda xato yuz berdi.")
